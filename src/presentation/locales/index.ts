import { createI18n } from 'vue-i18n'

const messages = {
  fa: {
    dashboard: {
      title: 'داشبورد',
      welcome: 'به سیستم مدیریت فرمولاسیون خوش آمدید'
    },
    materials: {
      title: 'مواد اولیه',
      addNew: 'افزودن ماده اولیه جدید',
      searchPlaceholder: 'جستجوی مواد اولیه...',
      loading: 'در حال بارگذاری...',
      noMaterials: 'هیچ ماده اولیه‌ای ثبت نشده است',
      addFirst: 'افزودن اولین ماده اولیه',
      edit: 'ویرایش',
      delete: 'حذف',
      confirmDelete: 'آیا مطمئن هستید که می‌خواهید',
      unit: 'واحد',
      dimension: 'بعد',
      currentPrice: 'قیمت فعلی',
      effectiveFrom: 'موثر از',
      noPrice: 'قیمتی ثبت نشده است',
      updatePrice: 'به‌روزرسانی قیمت',
      setPrice: 'ثبت قیمت',
      priceHistory: 'تاریخچه قیمت',
      current: 'فعلی',
      noPriceHistory: 'تاریخچه قیمتی وجود ندارد',
      createMaterial: 'ایجاد ماده اولیه',
      editMaterial: 'ویرایش ماده اولیه',
      name: 'نام',
      namePlaceholder: 'نام ماده اولیه',
      price: 'قیمت',
      toman: 'تومان',
      pricePlaceholder: 'قیمت به تومان',
      mass: 'جرم',
      volume: 'حجم',
      count: 'تعداد',
      cancel: 'لغو',
      save: 'ذخیره'
    },
    products: {
      title: 'محصولات',
      addNew: 'افزودن محصول جدید',
      searchPlaceholder: 'جستجوی محصولات...',
      loading: 'در حال بارگذاری...',
      noProducts: 'هیچ محصولی ثبت نشده است',
      addFirst: 'افزودن اولین محصول',
      edit: 'ویرایش',
      delete: 'حذف',
      confirmDelete: 'آیا مطمئن هستید که می‌خواهید',
      type: 'نوع',
      unit: 'واحد',
      dimension: 'بعد',
      yieldQty: 'مقدار تولید',
      description: 'توضیحات',
      images: 'تصاویر',
      addImage: 'افزودن تصویر',
      deleteImage: 'حذف تصویر',
      noImages: 'تصویری ثبت نشده است',
      confirmDeleteImage: 'آیا مطمئن هستید که می‌خواهید این تصویر را حذف کنید؟',
      createProduct: 'ایجاد محصول',
      editProduct: 'ویرایش محصول',
      name: 'نام',
      namePlaceholder: 'نام محصول',
      middle: 'میانی',
      final: 'نهایی',
      allTypes: 'همه انواع',
      mass: 'جرم',
      volume: 'حجم',
      count: 'تعداد',
      recipeBasis: 'مبنای فرمول',
      yieldQtyPlaceholder: 'مقدار تولید',
      yieldQtyHint: 'مقدار تولید محصول بر اساس این فرمول (حداکثر 5 رقم اعشار)',
      descriptionPlaceholder: 'توضیحات محصول (اختیاری)',
      cancel: 'لغو',
      save: 'ذخیره',
      selectImage: 'انتخاب تصویر',
      imageHint: 'حداکثر 3 تصویر، هر تصویر حداکثر 1 مگابایت',
      upload: 'آپلود',
      uploading: 'در حال آپلود...',
      costBreakdown: 'تجزیه هزینه',
      recalculate: 'محاسبه مجدد',
      totalCost: 'هزینه کل',
      costPerUnit: 'هزینه به ازای واحد',
      materialRequirements: 'نیازمندی‌های مواد اولیه',
      noCostData: 'اطلاعات هزینه در دسترس نیست',
      calculateCost: 'محاسبه هزینه'
    },
    bom: {
      selectProduct: 'انتخاب محصول',
      editor: 'ویرایشگر فرمول',
      addLine: 'افزودن خط',
      material: 'ماده اولیه',
      product: 'محصول',
      quantity: 'مقدار',
      unit: 'واحد',
      wastePct: 'درصد ضایعات',
      optional: 'اختیاری',
      delete: 'حذف',
      moveUp: 'جابجایی به بالا',
      moveDown: 'جابجایی به پایین',
      noLines: 'هیچ خطی در فرمول وجود ندارد',
      select: 'انتخاب کنید',
      selectMaterial: 'انتخاب ماده اولیه',
      add: 'افزودن',
      cancel: 'لغو',
      confirmDeleteLine: 'آیا مطمئن هستید که می‌خواهید این خط را حذف کنید؟',
      inputType: 'نوع ورودی',
      versions: 'نسخه‌های فرمول',
      createNewVersion: 'ایجاد نسخه جدید',
      version: 'نسخه',
      current: 'فعلی',
      noVersions: 'هیچ نسخه‌ای وجود ندارد',
      setAsCurrent: 'تنظیم به عنوان فعلی',
      editingVersion: 'ویرایش نسخه',
      saveAsCurrent: 'ذخیره به عنوان نسخه فعلی',
      confirmSetCurrent: 'آیا مطمئن هستید که می‌خواهید این نسخه را به عنوان نسخه فعلی تنظیم کنید؟',
      confirmSaveAsCurrent: 'آیا مطمئن هستید که می‌خواهید این نسخه را به عنوان نسخه فعلی ذخیره کنید؟',
      versionNotes: 'یادداشت نسخه',
      notesPlaceholder: 'یادداشت‌های این نسخه (اختیاری)',
      create: 'ایجاد',
      changeProduct: 'تغییر محصول',
      yield: 'مقدار تولید',
      middle: 'میانی',
      final: 'نهایی'
    },
    calculator: {
      title: 'ماشین حساب تولید',
      description: 'محاسبه مواد اولیه مورد نیاز برای تولید مقدار مشخصی از محصول',
      input: 'ورودی',
      selectProduct: 'انتخاب محصول',
      select: 'انتخاب کنید',
      targetQuantity: 'مقدار مورد نظر',
      quantityPlaceholder: 'مقدار',
      productYield: 'مقدار تولید محصول',
      calculate: 'محاسبه',
      calculating: 'در حال محاسبه...',
      export: 'خروجی JSON',
      results: 'نتایج',
      product: 'محصول',
      quantity: 'مقدار',
      totalCost: 'هزینه کل',
      material: 'ماده اولیه',
      requiredQuantity: 'مقدار مورد نیاز',
      unit: 'واحد',
      pricePerUnit: 'قیمت به ازای واحد',
      costContribution: 'سهم هزینه',
      noRequirements: 'هیچ ماده اولیه‌ای مورد نیاز نیست',
      calculationError: 'خطا در محاسبه',
      middle: 'میانی',
      final: 'نهایی'
    },
    gallery: {
      title: 'گالری محصولات',
      addProduct: 'افزودن محصول',
      searchPlaceholder: 'جستجوی محصولات...',
      loading: 'در حال بارگذاری...',
      loadError: 'خطا در بارگذاری محصولات',
      noProducts: 'هیچ محصولی ثبت نشده است',
      noResults: 'نتیجه‌ای یافت نشد',
      addFirst: 'افزودن اولین محصول',
      sortByUpdated: 'بروزرسانی',
      sortByName: 'نام',
      sortByCost: 'هزینه',
      descending: 'نزولی',
      ascending: 'صعودی',
      allCosts: 'همه هزینه‌ها',
      costRange1: '0 - 1,000 تومان',
      costRange2: '1,000 - 10,000 تومان',
      costRange3: '10,000 - 100,000 تومان',
      costRange4: '100,000+ تومان',
      cost: 'هزینه',
      notCalculated: 'محاسبه نشده',
      actions: 'عملیات',
      view: 'مشاهده',
      edit: 'ویرایش',
      duplicate: 'تکثیر محصول',
      duplicateWithBom: 'تکثیر با فرمول',
      createFromTemplate: 'ایجاد از قالب',
      delete: 'حذف',
      confirmDelete: 'آیا مطمئن هستید که می‌خواهید',
      duplicateError: 'خطا در تکثیر محصول',
      deleteError: 'خطا در حذف محصول',
      middle: 'میانی',
      final: 'نهایی'
    },
    settings: {
      title: 'تنظیمات',
      appInfo: 'اطلاعات برنامه',
      appName: 'نام برنامه',
      appVersion: 'نسخه',
      buildDate: 'تاریخ ساخت',
      storage: 'فضای ذخیره‌سازی',
      storageUsed: 'استفاده شده',
      storageQuota: 'سهمیه کل',
      storageUsage: 'درصد استفاده',
      refresh: 'بروزرسانی',
      pwa: 'حالت آفلاین',
      offlineMode: 'وضعیت اتصال',
      online: 'آنلاین',
      offline: 'آفلاین',
      updateAvailable: 'به‌روزرسانی جدید موجود است',
      updateNow: 'به‌روزرسانی کن',
      backupRestore: 'پشتیبان‌گیری و بازیابی',
      exportBackup: 'خروجی پشتیبان',
      importing: 'در حال وارد کردن...',
      exporting: 'در حال خروجی...',
      importBackup: 'وارد کردن پشتیبان',
      importSuccess: 'وارد کردن موفق: {materials} ماده اولیه، {products} محصول، {bomVersions} نسخه BOM',
      importError: 'خطا در وارد کردن پشتیبان',
      exportError: 'خطا در خروجی پشتیبان'
    }
  }
}

const i18n = createI18n({
  legacy: false,
  locale: 'fa',
  fallbackLocale: 'fa',
  messages
})

export default i18n
